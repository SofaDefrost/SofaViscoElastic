...
class KelvinVoigtSecondOrder : public BaseViscoelasticMaterial<DataTypes>{
    ...
    virtual void deriveSPKTensor(StrainInformation<DataTypes> *sinfo, const MaterialParameters<DataTypes> &param, MatrixSym &SPKTensorGeneral,MatrixSym &CauchyStressTensor, SReal& dt) override
    {
        ...
        Edot = (sinfo->E-Eprev)/dt;
        Real alpha = E1*tau1+E2*tau2;
        ...
        sinfo->Evisc2 = (1/(1+((E1+E2)*dt/alpha)))*(sinfo->Evisc_prev2+(E1*dt/alpha)*sinfo->E+(E1*tau1*dt/alpha)*Edot);
        sinfo->Evisc1 = (sinfo->E-sinfo->Evisc2);
        ...
        CauchyStressTensor =(E1+E2)*(sinfo->Evisc1+sinfo->Evisc2);
    ...
    virtual void applyElasticityTensor(StrainInformation<DataTypes> *sinfo, const MaterialParameters<DataTypes> &param,const MatrixSym& inputTensor, MatrixSym &outputTensor, SReal& dt) override
      {
        ...
        Real alpha = 1/(((1/E1)*(1-exp(-dt/tau1)))+((1/E2)*(1-exp(-dt/tau2))));
        Real alpha1 = 1/(((1/E1)*(1-exp(-dt/tau1)))+((1/E2)));
        Real alpha2 = 1/((1/E1)+((1/E2)*(1-exp(-dt/tau2))));
        Real alpha3 = 1/((1/E1)+((1/E2)));   
        ...
        for(int k = 0; k<3; ++k){
            for(int l=0; l<3; ++l){
                if(sinfo->Evdot1(k,l) >= (1/tau1) && sinfo->Evdot2(k,l) >= (1/tau2) ){
                    outputTensor = Thirdmatrix*(alpha3-(alpha3/(3*(1-2*nu)))*log(sinfo->J))*0.5 + 0.5* inversematrix*(alpha3/(3*(1-2*nu)))*trHC;
                }
                else if(sinfo->Evdot1(k,l) < (1/tau1) && sinfo->Evdot2(k,l) >= (1/tau2) ){

                    outputTensor = Thirdmatrix*(alpha1-((E1+E2)/(3*(1-2*nu)))*log(sinfo->J))*0.5 + 0.5*inversematrix*((E1+E2)/(3*(1-2*nu)))*trHC;

                }
                else if(sinfo->Evdot1(k,l) >= (1/tau1) && sinfo->Evdot2(k,l) < (1/tau2) ){

                    outputTensor = Thirdmatrix*(alpha2-(alpha2/(3*(1-2*nu)))*log(sinfo->J))*0.5 + 0.5*inversematrix*(alpha2/(3*(1-2*nu)))*trHC;

                }
                else{
                    outputTensor = Thirdmatrix*(alpha-(alpha/(3*(1-2*nu)))*log(sinfo->J))*0.5 + 0.5*inversematrix*(alpha/(3*(1-2*nu)))*trHC;
