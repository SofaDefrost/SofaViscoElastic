...
class SLSMaxwellSecondOrder : public BaseViscoelasticMaterial<DataTypes>{
    ...
    virtual void deriveSPKTensor(StrainInformation<DataTypes> *sinfo, const MaterialParameters<DataTypes> &param, MatrixSym &SPKTensorGeneral,MatrixSym &CauchyStressTensor, SReal& dt) override
    {
        ...
        CauchyStressTensor = E1*sinfo->E + E2*(sinfo->E-sinfo->Evisc1) + E3*(sinfo->E-sinfo->Evisc2);
        ...
        for(int k = 0; k<3; ++k){
            for(int l=0; l<3; ++l){
                if(sinfo->Evdot1(k,l) >= (1/tau2) && sinfo->Evdot2(k,l) >= (1/tau3) ){
                    ...
                    outputTensor = Thirdmatrix*(E1+E2*exp(-dt/tau2)+E3-((E1+E2+E3)/(3*(1-2*nu)))*log(sinfo->J))*0.5 + 0.5*inversematrix*((E1+E2+E3)/(3*(1-2*nu)))*trHC;

                }
                else if(sinfo->Evdot1(k,l) >= (1/tau2) && sinfo->Evdot2(k,l) < (1/tau3) ){

                    outputTensor = Thirdmatrix*(E1+E2+E3*exp(-dt/tau3)-((E1+E2+E3)/(3*(1-2*nu)))*log(sinfo->J))*0.5 + 0.5*inversematrix*((E1+E2+E3)/(3*(1-2*nu)))*trHC;
