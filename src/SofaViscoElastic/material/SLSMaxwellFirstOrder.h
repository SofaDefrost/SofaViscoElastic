...
class SLSMaxwellFirstOrder : public BaseViscoelasticMaterial<DataTypes>{
    ...
    void deriveSPKTensor(StrainInformation<DataTypes> *sinfo, const MaterialParameters<DataTypes> &param,MatrixSym &SPKTensorGeneral, Real& dt) override
    {
        ...
        SPKTensorGeneral = E0*E+(E0+E1)*tau*Edot-tau*sinfo->SPKdot;
    ...
    void applyElasticityTensor(StrainInformation<DataTypes> *sinfo, const MaterialParameters<DataTypes> &param,const MatrixSym& inputTensor, MatrixSym &outputTensor, Real& dt) override
