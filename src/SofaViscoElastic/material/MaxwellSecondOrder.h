...
/* a Class that describe a generic Viscoelastic material : example of  Maxwell Second Order.
...
class MaxwellSecondOrder : public BaseViscoelasticMaterial<DataTypes>{
    ...
    virtual void deriveSPKTensor(StrainInformation<DataTypes> *sinfo, const MaterialParameters<DataTypes> &param, MatrixSym &SPKTensorGeneral,MatrixSym &CauchyStressTensor, SReal& dt) override
    {
        ...
        sinfo->Evisc2 = (1/(1+(dt/tau2)))*(sinfo->Evisc_prev2+ (dt/tau2)*sinfo->E);
        ...
        CauchyStressTensor = E1*(sinfo->E-sinfo->Evisc1) + E2*(sinfo->E-sinfo->Evisc2);
        ...
        for(int k = 0; k<3; ++k){
            for(int l=0; l<3; ++l){
                if(sinfo->Evdot1(k,l) >= (1/tau1) && sinfo->Evdot2(k,l) >= (1/tau2) ){
                    outputTensor = Thirdmatrix*(E1+E2-((E1+E2)/(3*(1-2*nu)))*log(sinfo->J))*0.5 + 0.5*inversematrix*((E1+E2)/(3*(1-2*nu)))*trHC;
                }
                else if(sinfo->Evdot1(k,l) < (1/tau1) && sinfo->Evdot2(k,l) >= (1/tau2) ){

                    outputTensor = Thirdmatrix*(E1*exp(-dt/tau1)+E2-((E1+E2)/(3*(1-2*nu)))*log(sinfo->J))*0.5+ 0.5*inversematrix*((E1+E2)/(3*(1-2*nu)))*trHC;

                }
                else if(sinfo->Evdot1(k,l) >= (1/tau1) && sinfo->Evdot2(k,l) < (1/tau2) ){

                    outputTensor = Thirdmatrix*(E1+E2*exp(-dt/tau2)-((E1+E2)/(3*(1-2*nu)))*log(sinfo->J))*0.5 + 0.5*inversematrix*((E1+E2)/(3*(1-2*nu)))*trHC;

                }
                else{
                    outputTensor = Thirdmatrix*(E1*exp(-dt/tau1)+E2*exp(-dt/tau2)-((E1+E2)/(3*(1-2*nu)))*log(sinfo->J))*0.5 + 0.5*inversematrix*((E1+E2)/(3*(1-2*nu)))*trHC;
