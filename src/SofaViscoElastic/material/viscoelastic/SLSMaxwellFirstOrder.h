...
public:
  ...
  static constexpr std::string_view Name = "SLSMaxwellFirstOrder";
    ...
    void deriveCauchyGreenStressTensor(StrainInformation<DataTypes> *sinfo, const MaterialParameters<DataTypes> &param, MatrixSym &CauchyStressTensor, SReal& dt) override
    {
        ...
        sinfo->Evisc1 = (1 / (1 + ( tau / dt ))) * (( tau / dt )* sinfo->Evisc_prev1 + sinfo->Edev);
        
        MatrixSym Ediff = sinfo->E-sinfo->Evisc1;

        /// The equation of the Cauchy Stress tensor for the SLS Maxwell Model.
        CauchyStressTensor = 2 * G0 * sinfo->Edev + 2 * G1 * Ediff + lambda * sinfo->trE * ID;
