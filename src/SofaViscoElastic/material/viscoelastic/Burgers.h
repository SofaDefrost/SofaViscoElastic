...
public:
  ...
  typedef type::MatSym<3,Real> MatrixSym;
	...
	virtual void deriveCauchyGreenStressTensor(StrainInformation<DataTypes> *sinfo, const MaterialParameters<DataTypes> &param,MatrixSym &CauchyStressTensor, SReal& dt) override
    {
        ...
        sinfo->Evisc1 = (1 / (G1 + (G1 * tau1 / dt))) * (G1 * sinfo->Edev + (G1 * tau1 / dt) * sinfo->Evisc_prev1 - G1 * sinfo->Evisc_prev2);
        sinfo->Evisc2 = (1 / (G1 + G2 + (G2 * tau2 / dt))) * (G1 *sinfo->Edev + (G2 * tau2 / dt) * sinfo->Evisc_prev2 - G1 * sinfo->Evisc_prev1)  ;

				 /// The equation of the Cauchy-Green Stress tensor for the Maxwell Model.
        CauchyStressTensor = 2* G1 * (sinfo->Edev - sinfo->Evisc1 - sinfo->Evisc2) + lambda * sinfo->trE * ID;
